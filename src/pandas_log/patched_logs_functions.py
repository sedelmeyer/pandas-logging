"""
pandas_log.patched_log_functions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. note::

   The ``pandas_log.patched_log_functions`` module is copied directly from
...
# Values messages
ALTERED_VALUES_MSG = (
    "\t* Changed {values_changed} values, {values_unchanged} values "
    "were not changed."
)
...
FILTERED_ROWS_MSG = (
    "\t* Removed {rows_removed} rows ({rows_removed_pct}%), "
...
FILTERED_COLS_MSG = (
    "\t* Removed the following columns ({cols_removed}) now only have the following "
    "columns ({cols_remaining})."
)
...
# TIPS
ITERROWS_TIPS = (
    "\t*iterrows is not recommended, and in the majority of cases will "
    "have better alternatives"
)
FILLNA_NO_NA_TIP = (
    "\t* There are no nulls in this dataframe, if you are working on the entire "
    "dataset you can remove this operation."
)
SHOULD_REDUCED_ROW_TIP = (
    "\t* Number of rows didn't change, if you are working on the entire dataset you "
    "can remove this operation."
)

# Others
SORT_VALUES_MSG = "\t* Sorting by columns {} in a {} order."
SORT_INDEX_MSG = (
    "\t* Sorting by index in a "
    "{'ascending' if ascending else 'descending'} order."
)
...
COPY_WARNING_MSG = (
    "Some pandas logging may involve copying dataframes, which can be "
    "time-/memory-intensive. Consider passing copy_ok=False to the enable/auto_enable "
    "functions in pandas_log if issues arise."
)
...
def num_values_changed(input_obj, output_obj):
    if (
        ...
        # Comparing values for equality across dtypes wouldn't be well-defined so we
        # just say they all changed
...
def log_assign(output_df, input_df, **kwargs):
    ...
    if changed_cols:
        if kwargs["copy_ok"]:
            ...
            # (Doing the above calculation would always say zero values changed in this
            # case, since if copying isn't ok then input_df and output_df point to the
            # same object)
            logs.append(
...
def log___setitem__(output_df, input_df, key, value, **kwargs):
    if isinstance(key, str):
        ...
        # This would be kind of complicated but since we don't actually use the values
        # in kwargs we can just add the new columns as keys each with the full set of
        # assigned values (as a placeholder)
        for subkey in key:
            kwargs[subkey] = value
...
def log_mask(
    ...
    **kwargs,
    ...
    if isinstance(cond, pd.Series) and isinstance(input_df, pd.Series):
        # Calculate the values for which the condition was true but the value didn't
        # change only for this simplest case, where we can just & the two series
        true_but_unchanged = (cond & (output_df == input_df)).sum()
        if true_but_unchanged > 0:
            logs.append(
                "\t* {} rows met the masking condition and already had the masking "
                "value.".format(true_but_unchanged)
            )
    logs = "\n".join(logs)
    tips = ""
    return logs, tips
