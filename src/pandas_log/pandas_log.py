...
.. note::
   ...
   The ``pandas_log.pandas_log`` module is copied directly from
...
def auto_disable():
    """ Restore original pandas method without the additional log functionality
    (statistics)
...
def enable(
    ...
    """ Adds the additional logging functionality (statistics) to pandas methods only
    for the scope of this context manager.
        ...
        :param copy_ok: whether the dataframe is allowed to be copied to calculate more
                        informative metadata logs
        :return: None
    """
...
def create_overide_pandas_func(
    ...
    """ Create overridden pandas method dynamically with
        ...
        Note: if we extracting _overide_pandas_method outside we need to implement
              decorator like here
    ...
    def _run_method_and_calc_stats(
        ...
        if copy_ok:
            # If we're ok to make copies, copy the input_df so that we can compare
            # against the output of inplace methods
            try:
                # Will hit infinite recursion if we use the patched copy method so
                # use the original
        ...
        if output_df is None:
            # The operation was strictly in place so we just call the dataframe the
            # output_df as well
            output_df = input_df
        if copy_ok:
            # If this isn't true and the method was strictly inplace, input_df and
            # output_df will just
